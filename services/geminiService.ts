...
import { DocumentType, Status, Document } from '../types';
...
export const parseTextToDocument = async (text: string): Promise<Partial<any>> => {
    if (!window.APP_CONFIG?.API_KEY || window.APP_CONFIG.API_KEY.includes('PASTE_YOUR_GEMINI_API_KEY_HERE')) {
...
export const generateReportFromData = async (documents: Document[]): Promise<string> => {
    if (!window.APP_CONFIG?.API_KEY || window.APP_CONFIG.API_KEY.includes('PASTE_YOUR_GEMINI_API_KEY_HERE')) {
        throw new Error("Gemini API key is not configured. Please edit 'index.html' to add your API key.");
    }
    ...
    const prompt = `
        ...
        4.  **Urgent Items:** Specifically list any documents with a "Due" status, indicating which ones need immediate attention.
        5.  **Suggestions:** Provide one or two actionable suggestions for improving their document workflow based on the data. For example, if many items are due, suggest they focus on payments. If many requisitions are pending, suggest they follow up with suppliers for quotes.
