...
const LogoUploadCard = ({ title, description, currentLogoUrl, onSave, isSaving }) => {
...
const PaginationControls = ({ currentPage, totalPages, onNext, onPrev, totalItems, rowsPerPage }) => {
    if (totalPages <= 1) return null;
    return React.createElement('div', { className: 'p-4 flex items-center justify-between text-sm text-gray-600 dark:text-gray-400' },
        React.createElement('span', null, `Showing ${((currentPage - 1) * rowsPerPage) + 1}-${Math.min(currentPage * rowsPerPage, totalItems)} of ${totalItems}`),
        React.createElement('div', { className: 'flex items-center gap-2' },
            React.createElement('button', { onClick: onPrev, disabled: currentPage === 1, className: 'p-2 border border-gray-300 dark:border-gray-600 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-100 dark:hover:bg-gray-600' }, React.createElement(Icon, { name: 'chevron-left', className: 'w-4 h-4' })),
            React.createElement('span', null, `Page ${currentPage} of ${totalPages}`),
            React.createElement('button', { onClick: onNext, disabled: currentPage === totalPages, className: 'p-2 border border-gray-300 dark:border-gray-600 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-100 dark:hover:bg-gray-600' }, React.createElement(Icon, { name: 'chevron-right', className: 'w-4 h-4' }))
        )
    );
};

export const AdminConsolePage = ({ currentUser, settings, onSettingsUpdate }) => {
    ...
    const userTotalPages = Math.ceil(users.length / rowsPerPage);
    ...
    const entityTotalPages = Math.ceil(entities.length / rowsPerPage);
    const paginatedEntities = useMemo(() => {
        const startIndex = (entityCurrentPage - 1) * rowsPerPage;
        return entities.slice(startIndex, startIndex + rowsPerPage);
    }, [entities, entityCurrentPage]);
            ...
            React.createElement('div', { className: "space-y-8" },
                ...
                React.createElement('div', null,
                 ...
                 React.createElement('div', { className: "bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden" },
                    ...
                    React.createElement(PaginationControls, { currentPage: entityCurrentPage, totalPages: entityTotalPages, onNext: () => setEntityCurrentPage(p => p+1), onPrev: () => setEntityCurrentPage(p => p-1), totalItems: entities.length, rowsPerPage })
                ),
                ...
                React.createElement('div', { className: "bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden" },
                    ...
                    React.createElement(PaginationControls, { currentPage: userCurrentPage, totalPages: userTotalPages, onNext: () => setUserCurrentPage(p => p+1), onPrev: () => setUserCurrentPage(p => p-1), totalItems: users.length, rowsPerPage })
                )
            ),
