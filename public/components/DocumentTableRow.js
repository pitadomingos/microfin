...
export const DocumentTableRow = ({ doc, onEdit, onDelete }) => {
    ...
    const isOverdue = doc.dueDate && new Date(doc.dueDate) < new Date() && doc.status !== Status.PAID;
    ...
    return (
        React.createElement('tr', { className: "hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors duration-150 text-sm text-gray-700 dark:text-gray-300" },
            ...
            React.createElement('td', { className: "px-4 py-3 font-mono text-right" }, formatAmount(doc.amount)),
            React.createElement('td', { className: `px-4 py-3 whitespace-nowrap ${isOverdue ? 'text-red-500 font-semibold' : ''}` }, formatDate(doc.dueDate)),
            React.createElement('td', { className: "px-4 py-3" },
